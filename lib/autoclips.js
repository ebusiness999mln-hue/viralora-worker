...
async function renderAutoClips(job, supabase) {
  ...
  const clips = []
  const debug = [] // ponytail: capture failure reasons — we're blind to Railway logs
  for (let i = 0; i < picks.length; i++) {
    ...
    try {
      // tv/web_safari clients still serve classic (non-SABR) format URLs that
      // ffmpeg can range-download; android got SABR-gated on datacenter IPs
      // (yt-dlp#12482). Try those first, fall through to the rest. No proxy/cookies.
      // Resilient format falls back to best available if 480p isn't offered.
      // No --force-keyframes-at-cuts: it makes yt-dlp's internal ffmpeg build an
      // atrim filter with a negative duration on some streams ("durationi out of
      // range" → exit 1). A plain section cut starts at the nearest keyframe —
      // fine for clips and reliable.
      await run('yt-dlp', [
        '--extractor-args', 'youtube:player_client=tv,web_safari,ios,android,web',
      ...
      if (debug.length < 2) debug.push(`clip${i}: ${String(e.stderr || e.message || e).slice(-700)}`)
  ...
  return { clips, debug: { v: 'cuts-v4', errors: debug } }
}
