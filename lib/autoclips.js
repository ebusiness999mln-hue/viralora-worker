...
async function renderAutoClips(job, supabase) {
  ...
  for (let i = 0; i < picks.length; i++) {
    ...
    try {
      // player_client=android dodges the "confirm you're not a bot" wall the web
      // client hits on datacenter IPs (no cookies/proxy needed). Resilient format
      // falls back to best available if 480p isn't offered.
      await run('yt-dlp', [
      ...
      if (debug.length < 2) debug.push(`clip${i}: ${String(e.stderr || e.message || e).slice(0, 600)}`)
