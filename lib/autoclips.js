...
async function renderAutoClips(job, supabase) {
  ...
  // hqdefault always exists; maxresdefault 404s when the uploader gave no HD
  // thumbnail, which is why posters/fallback thumbs were showing broken.
  ...
  for (let i = 0; i < picks.length; i++) {
    ...
    const raw = `${tmp}/raw_${i}.mp4`
    const vert = `${tmp}/vert_${i}.mp4`
    try {
      if (!sourceOk) throw new Error('source video unavailable')

      // Step 1 — cut [start,end] with stream copy (fast, no re-encode). This is
      // the reliable part: if it works we always have a usable clip to upload.
      await run('ffmpeg', [
        '-y', '-ss', String(w.start), '-i', source, '-t', String(w.end - w.start),
        '-c', 'copy', '-movflags', '+faststart', raw,
      ], { timeout: 120000 })
      if (!(await hasVideo(raw))) throw new Error('cut produced no video stream')

      // Step 2 — convert to vertical 9:16 1080x1920 + burn caption, as a SEPARATE
      // pass. floor(.../2)*2 keeps the crop width even so libx264 won't reject it.
      // If this fails for any reason, fall back to the Step 1 cut — never fail the
      // whole clip just because the vertical reframe didn't take.
      let final = raw
      try {
        await run('ffmpeg', [
          '-y', '-i', raw,
          '-vf', `crop=floor(in_h*9/16/2)*2:in_h,scale=1080:1920,drawtext=fontfile=${FONT}:text='${p.caption}':fontsize=56:fontcolor=white:x=(w-text_w)/2:y=h-320:box=1:boxcolor=black@0.5`,
          '-c:v', 'libx264', '-preset', 'veryfast', '-c:a', 'aac', '-movflags', '+faststart', vert,
        ], { timeout: 120000 })
        if (await hasVideo(vert)) final = vert
        else console.error(`clip ${i} vertical pass produced no video, using horizontal cut`)
      } catch (e) {
        console.error(`clip ${i} vertical convert failed, using horizontal cut:`, String(e.stderr || e.message || e).slice(-300))
      }
