...
async function renderAutoClips(job, supabase) {
  ...
  for (let i = 0; i < picks.length; i++) {
    ...
    try {
      ...
      // Cut [start,end] from the local file, crop center to 9:16, scale to
      // 1080x1920, and burn the caption in one pass. -ss before -i = fast seek;
      // re-encode so the cut is frame-accurate. crop=ih*9/16:ih:(iw-ih*9/16)/2:0
      // takes the center vertical slice; scale fixes the TikTok/Reels/Shorts size.
      await run('ffmpeg', [
        '-y', '-ss', String(w.start), '-i', source, '-t', String(w.end - w.start),
        '-vf', `crop=ih*9/16:ih:(iw-ih*9/16)/2:0,scale=1080:1920,drawtext=fontfile=${FONT}:text='${p.caption}':fontsize=56:fontcolor=white:x=(w-text_w)/2:y=h-320:box=1:boxcolor=black@0.5`,
