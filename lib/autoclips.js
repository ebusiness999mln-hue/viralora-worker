...
// job.script.moments. We fetch the source once via Cobalt, cut each section
// with FFmpeg, burn a caption, and upload the MP4 to Supabase Storage. Any
// per-clip failure (or no source) falls back to a YouTube deep-link so the
// user always gets something. No Python, no ML — Cobalt + ffmpeg only.
...
// Download the source video via a self-hosted Cobalt instance (COBALT_URL).
// Cobalt resolves YouTube from a clean IP and returns a direct media URL,
// dodging the datacenter-IP gating that blocks yt-dlp on Railway.
async function downloadWithCobalt(videoUrl, outputPath) {
  const cobaltUrl = process.env.COBALT_URL || 'https://positive-charm-production.up.railway.app'
  ...
  // Cobalt v10 returns status 'tunnel'; older builds 'stream'. Both, plus
  // 'redirect', carry a direct media URL in data.url.
  ...
  throw new Error('Cobalt download failed: ' + JSON.stringify(data))
}
...
async function renderAutoClips(job, supabase) {
  ...
  // Fetch the full source video ONCE via Cobalt, then cut every section locally
  // with ffmpeg (100% reliable on a local file). Cobalt runs on a clean IP, so
  // it dodges the datacenter-IP gating that breaks yt-dlp on Railway.
  const source = `${tmp}/source.mp4`
  let sourceOk = false
  try {
    await downloadWithCobalt(videoUrl, source)
    sourceOk = await hasVideo(source)
    if (!sourceOk) console.error('source downloaded but has no video stream')
  } catch (e) {
    console.error('source download failed:', String(e.message || e).slice(-500))
  }
