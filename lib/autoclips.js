...
async function renderAutoClips(job, supabase) {
  ...
  try {
    ...
    if (!sourceOk) debug.push(`source: downloaded but no video stream`)
  } catch (e) {
    debug.push(`source: ${String(e.stderr || e.message || e).slice(-500)}`)
  }
  ...
  for (let i = 0; i < picks.length; i++) {
    ...
    try {
      ...
      await run('ffmpeg', [
        '-y', '-ss', String(p.start), '-i', source, '-t', String(p.end - p.start),
      ...
      if (debug.length < 3) debug.push(`clip${i}: ${String(e.stderr || e.message || e).slice(-400)}`)
  ...
  return { clips, debug: { v: 'cuts-v5', errors: debug } }
}
