...
// The cloned shorts-generator downloads via yt-dlp's Python API, which ignores
// config files and exposes no --cookies flag, so the only working hook is its
// ydl_opts dict. Inject a cookiefile read from $YTDLP_COOKIES. Runtime +
// idempotent so it self-heals even if the Docker build-time patch didn't apply.
function ensureCookiePatch() {
  const f = '/app/shorts-generator/shorts_generator/local/downloader.py'
  try {
    const src = fs.readFileSync(f, 'utf8')
    if (src.includes('YTDLP_COOKIES')) return true // already patched (build or prior run)
    const anchor = '"format": _format_for(fmt),'
    if (!src.includes(anchor)) {
      console.error('cookie patch: anchor not found in downloader.py — upstream moved it')
      return false
    }
    fs.writeFileSync(f, src.replace(anchor, `${anchor}\n        "cookiefile": os.environ.get("YTDLP_COOKIES") or None,`))
    return true
  } catch (e) {
    console.error('cookie patch failed:', e.message)
    return false
  }
}
...
async function renderAutoClips(job, supabase) {
  ...
  try {
    ...
    if (process.env.COOKIE_BASE64) {
      ...
      ensureCookiePatch() // make the generator's yt-dlp honor YTDLP_COOKIES
    }
    ...
    execSync(
      ...
      {
        timeout: 300000,
        // Point the generator's "OpenAI" client at OpenRouter (free models). Its
        // client passes no base_url, so the openai SDK reads OPENAI_BASE_URL from
        // env; OPENAI_API_KEY/OPENAI_MODEL come from its config. Needs
        // OPENROUTER_API_KEY set on Railway.
        env: {
          ...process.env,
          ...cookieEnv,
