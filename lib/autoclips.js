...
async function renderAutoClips(job, supabase) {
  ...
  try {
    ...
    execSync(
      ...
      {
        timeout: 300000,
        // Point the generator's "OpenAI" client at Groq. Its client passes no
        // base_url, so the openai SDK reads OPENAI_BASE_URL from env;
        // OPENAI_API_KEY/OPENAI_MODEL come from its config. Needs GROQ_API_KEY
        // set on Railway.
        env: {
          ...process.env,
          ...cookieEnv,
